...
class PostgreSQLClient:
    ...
    def from_env():
        ...
        database = os.environ.get("assistant_PG_DATABASE", "database")
        dbprop = config.get_section(database)
        if not dbprop:
            raise ValueError(f"{database} is not configured in .assistant.env.toml")
        if "url" in dbprop:
    ...
    def _parse_database_url(self, database_url: str):
        ...
        try:
            ...
            self.logger.info(
                f"Parsed database URL: {self.user}@{self.host}:{self.port}/{self.database}"
            )
    ...
    def execute_query(
        ...
        try:
            ...
            self.logger.info(
                f"Query executed successfully. Rows returned: {len(result_list)}"
            )
            return result_list
    ...
    def execute_update(self, query: str, params: Optional[tuple] = None) -> int:
        ...
        try:
            ...
            self.logger.info(
                f"Update executed successfully. Rows affected: {rows_affected}"
            )
    ...
    def execute_many(self, query: str, params_list: List[tuple]) -> int:
        ...
        try:
            ...
            self.logger.info(
                f"Batch execution successful. Rows affected: {rows_affected}"
            )
            return rows_affected
    ...
    def _display_table(
        self, results: List[Dict[str, Any]], title: str = "Query Results"
    ):
        """
        ...
        if not results:
            self.console.print(
                Panel(f"[yellow]No results returned[/yellow]", title=title)
            )
            return
    ...
    def execute_sql_script(
        self, sql_script: str, display_results: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        connection = None
        cursor = None
        summary = {"total_statements": 0, "successful": 0, "failed": 0, "results": []}
        ...
        try:
            ...
            for idx, statement in enumerate(statements, 1):
                try:
                    ...
                    if self._is_select_query(statement):
                        ...
                        if display_results:
                            self._display_table(
                                result_list, f"Results - Statement {idx}"
                            )
                        ...
                        summary["results"].append(
                            {
                                ...
                                "statement": statement,
                                "rows_affected": (
                                    rows_affected if rows_affected >= 0 else 0
                                ),
                            }
                        )
                    ...
                    if display_results:
                        self.console.print(
                            f"[red]✗ Error executing statement {idx}:[/red]"
                        )
            ...
            if display_results:
                ...
                summary_text.append(
                    f"Total Statements: {summary['total_statements']}\n"
                )
                summary_text.append(
    ...
    def execute_sql_file(
        self, file_path: str, display_results: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if display_results:
                self.console.print(
                    f"[bold blue]Reading SQL file: {file_path}[/bold blue]\n"
                )
    ...
    def get_table_foreign_keys(self, table_name: str) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(
                f"Error retrieving foreign keys for table '{table_name}': {e}"
            )
            raise
    ...
    def get_table_primary_key(self, table_name: str) -> List[str]:
        ...
        try:
            ...
            self.logger.error(
                f"Error retrieving primary key for table '{table_name}': {e}"
            )
            raise

    def get_table_schema(
        self, table_name: str, enhanced: bool = False
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            try:
                ...
                from ..prompts.sql import TABLE_DESCRIPTION_PROMPT
                from ..utils import json_utils

                llm_client = create_llm_client()
                descriptions = self.get_column_descriptions(
                    schema, table_name, select_columns
                )
                if descriptions:

                    table_mds = schema_lines + ["\n**Column Metadata:**"] + descriptions
                    tb_desc_prompt = TABLE_DESCRIPTION_PROMPT.format(
                        table_info="\n".join(table_mds)
                    )
                    resp_str = ""
                    for chunk in llm_client.invoke_model_generator(
                        prompt=tb_desc_prompt
                    ):
                        resp_str += chunk
                ...
                self.logger.error(
                    f"Error enhancing schema for table '{table_name}': {e}"
                )
    ...
    def get_columns_data_type(
        ...
        try:
            ...
            self.logger.error(
                f"Error retrieving data types for columns in table '{table_name}': {e}"
            )
            raise
    ...
    def get_column_descriptions(
        self, schema: str, table_name: str, select_columns: List[str]
    ) -> List[str]:
        """
        ...
        from ..llm.utils import create_llm_client
        from ..prompts.sql import COLUMN_DESCRITOPN_PROMPT
                ...
                for column, col_type in column_chunks:
                    ...
                    try:
                        sample_values = json.dumps(
                            [row[column] for row in results], cls=CustomJsonEncoder
                        )
                    except Exception as e:
                        sample_values = str([row[column] for row in results])

                    columns_info.append(
                        f"- Column Name: {column}\n - Column Data Type: {col_type}\n - Sample Values: {sample_values}\n"
                    )

                columns_info_str = "\n".join(columns_info)

                prompt = COLUMN_DESCRITOPN_PROMPT.format(
                    column_name=column, columns_info=columns_info_str
                )
                response_text = ""
                for chunk in llm_client.invoke_model_generator(prompt=prompt):
                    response_text += chunk
                    print(chunk, end="", flush=True)
...
if __name__ == "__main__":
    print("=== Example 1: Execute SQL Script with Multiple Statements ===\n")
    ...
    INSERT INTO employees (name, age, department) VALUES ('Bob', 35, 'Marketing');
    ...
    with PostgreSQLClient(
        ...
        summary = client.execute_sql_script(sql_script)

        print("\n=== Programmatic Access to Results ===")
        for result in summary["results"]:
            if result["type"] == "SELECT":
                print(
                    f"Statement {result['statement_number']} returned {result['rows_returned']} rows"
                )

    print("\n=== Example 2: Execute SQL from File ===\n")

    # Create a sample SQL file
    with open("sample_script.sql", "w") as f:
        f.write(sql_script)
    ...
    with PostgreSQLClient(
        ...
        summary = client.execute_sql_file("sample_script.sql")

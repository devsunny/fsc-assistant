...
class CRUDApiGenerator:
    ...
    def _add_jinja_filters(self):
        ...
        self.env.filters["sql_to_python_type"] = self._sql_type_to_python_type
        self.env.filters["get_pydantic_type"] = self._get_pydantic_type
        self.env.filters["get_sqlalchemy_type"] = self._get_sqlalchemy_type
        ...
        self.env.filters["should_use_server_default"] = self._should_use_server_default
        self.env.filters["get_child_tables"] = self._get_child_tables
    ...
    def _sql_type_to_python_type(self, column: Column) -> str:
    ...
    def _get_pydantic_type(self, column: Column) -> str:
        """Returns the Pydantic type string (e.g., str, Optional[int])."""
        py_type = self._sql_type_to_python_type(column)
        if py_type == "date":
    ...
    def _get_sqlalchemy_type(self, column: Column) -> str:
    ...
    def _get_pk_type(self, table: Table) -> str:
        ...
        if pk_column:
            return self._sql_type_to_python_type(pk_column)
    ...
    def _should_use_server_default(self, column: Column) -> bool:
        """Determines if a column should use server_default=func.now()."""
        data_type = column.data_type.lower()
        # Apply server_default for non-nullable date/datetime/timestamp columns without an explicit default
        return (
            data_type in ["date", "timestamp", "timestamptz", "datetime"]
...
# model.py.j2
MODEL_TEMPLATE = """
from sqlalchemy import Column, Integer, String, Boolean, DateTime, Text, ForeignKey, Numeric, Date, UUID, LargeBinary, JSON, func
...
class {{ table_pascal_case }}(Base):
    ...
    {{ column.name | snake_case }}: Mapped[{{ column | sql_to_python_type }}] = mapped_column(
        ...
        nullable=False,
        {% endif %}
        {% if column.default_value and not column | should_use_server_default %}
        default={{ column.default_value | replace("'", "") }},
        {% endif %}
        {% if column | should_use_server_default %}
    ...
    def __repr__(self):
        return f"<{{ table_pascal_case }}({{ pk_name }}={{ pk_column.name | snake_case }})>"
"""
...
class {{ table_pascal_case }}Update({{ table_pascal_case }}Base):
    {% for column in columns %}
    {% if not column.is_primary %}
    {{ column.name | snake_case }}: Optional[{{ column | get_pydantic_type }}] = None
    {% endif %}
    {% endfor %}

# Read Schema (basic, without relations)
class {{ table_pascal_case }}Read({{ table_pascal_case }}Base):
    {{ pk_name | snake_case }}: {{ pk_type }}
...
def get_{{ table_snake_case }}(db: Session, {{ pk_name | snake_case }}: {{ pk_type }}, eager_load_relations: bool = False) -> Optional[{{ table_pascal_case }}]:
    """
...
def update_{{ table_snake_case }}(db: Session, {{ pk_name | snake_case }}: {{ pk_type }}, {{ table_snake_case }}_update: {{ table_pascal_case }}Update) -> Optional[{{ table_pascal_case }}Read]:
    """
...
def delete_{{ table_snake_case }}(db: Session, {{ pk_name | snake_case }}: {{ pk_type }}) -> Optional[{{ table_pascal_case }}Read]:
    """
...
@router.get("/{{ '{' }}{{ pk_name | snake_case }}{{ '}' }}", response_model={{ table_pascal_case }}) # Use the full schema for single item
def read_{{ table_snake_case }}(
    ...
    Retrieve a single {{ table_snake_case }} by its primary key.
    """
    db_{{ table_snake_case }} = crud.get_{{ table_snake_case }}(db, {{ pk_name | snake_case }}={{ pk_name | snake_case }}, eager_load_relations=eager_load_relations)
...
@router.put("/{{ '{' }}{{ pk_name | snake_case }}{{ '}' }}", response_model={{ table_pascal_case }}Read)
def update_existing_{{ table_snake_case }}({{ pk_name | snake_case }}: {{ pk_type }}, {{ table_snake_case }}_update: {{ table_pascal_case }}Update, db: Session = Depends(get_db)):
    """
    Update an existing {{ table_snake_case }} record.
    """
    db_{{ table_snake_case }} = crud.update_{{ table_snake_case }}(db, {{ pk_name | snake_case }}={{ pk_name | snake_case }}, {{ table_snake_case }}_update={{ table_snake_case }}_update)
    if db_{{ table_snake_case }} is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="{{ table_pascal_case }} not found")
    return db_{{ table_snake_case }}

@router.delete("/{{ '{' }}{{ pk_name | snake_case }}{{ '}' }}", response_model={{ table_pascal_case }}Read)
def delete_existing_{{ table_snake_case }}({{ pk_name | snake_case }}: {{ pk_type }}, db: Session = Depends(get_db)):
    """
    Delete a {{ table_snake_case }} record.
    """
    db_{{ table_snake_case }} = crud.delete_{{ table_snake_case }}(db, {{ pk_name | snake_case }}={{ pk_name | snake_case }})
    if db_{{ table_snake_case }} is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="{{ table_pascal_case }} not found")
    return db_{{ table_snake_case }}
'''
...
updated_{{ table_snake_case }}_data = {
    {% for column in columns %}
    {% if not column.is_primary %}
    "{{ column.name | snake_case }}": {{ column | get_default_value_for_type | replace('_test', '_updated') | replace('1', '2') | replace('1.0', '2.0') | replace('2024-01-01', '2024-01-02') | replace('12:00:00Z', '13:00:00Z') }},
    {% endif %}
    {% endfor %}
}
...
async def test_create_{{ table_snake_case }}(test_client):
    ...
    data = response.json()
    assert data["{{ pk_name | snake_case }}"] is not None
    {% for column in columns %}
    {% if not column.is_primary %}
...
async def test_read_single_{{ table_snake_case }}(test_client):
    ...
    {{ pk_name | snake_case }} = create_response.json()["{{ pk_name | snake_case }}"]
    
    response = await test_client.get("/" + "{{ table_snake_case | pluralize }}" + "/" + str({{ pk_name | snake_case }}))
    assert response.status_code == 200
    data = response.json()
    assert data["{{ pk_name | snake_case }}"] == {{ pk_name | snake_case }}
    {% for column in columns %}
    {% if not column.is_primary %}
    assert data["{{ column.name | snake_case }}"] == sample_{{ table_snake_case }}_data["{{ column.name | snake_case }}"]
    {% endif %}
...
async def test_update_{{ table_snake_case }}(test_client):
    ...
    response = await test_client.put("/" + "{{ table_snake_case | pluralize }}" + "/" + str({{ pk_name | snake_case }}), json=updated_{{ table_snake_case }}_data)
    assert response.status_code == 200
    data = response.json()
    assert data["{{ pk_name | snake_case }}"] == {{ pk_name | snake_case }}
    {% for column in columns %}
...
async def test_delete_{{ table_snake_case }}(test_client):
    ...
    response = await test_client.delete("/" + "{{ table_snake_case | pluralize }}" + "/" + str({{ pk_name | snake_case }}))
    assert response.status_code == 200
    assert response.json()["{{ pk_name | snake_case }}"] == {{ pk_name | snake_case }}

    # Verify it's deleted
    get_response = await test_client.get("/" + "{{ table_snake_case | pluralize }}" + "/" + str({{ pk_name | snake_case }}))
...
services:
  web:
    ...
    command: ["/bin/bash", "-c", "python -c 'from database import create_db_and_tables; create_db_and_tables()' && uvicorn main:app --host 0.0.0.0 --port 8000"]

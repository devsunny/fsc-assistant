...
def init_command(tools: Optional[str], interactive: bool) -> None:
    ...
    Examples:
        ...
        fsc-assistant openspec init

        # Initialize with specific tools
        fsc-assistant openspec init --tools claude,cursor

        # Initialize with all tools
        fsc-assistant openspec init --tools all

        # Interactive mode (OpenSpec will prompt for input)
        fsc-assistant openspec init --interactive
...
def list_command(interactive: bool) -> None:
    ...
    Examples:
        ...
        fsc-assistant openspec list

        # List tools (non-interactive)
        fsc-assistant openspec list --no-interactive
...
def run_command(openspec_args: tuple, interactive: bool) -> None:
    ...
    Examples:
        ...
        # Run openspec help
        fsc-assistant openspec run --help

        # Run openspec with custom arguments
        fsc-assistant openspec run validate --strict

        # Run openspec config
        fsc-assistant openspec run config --show

        # Non-interactive mode
        fsc-assistant openspec run status --no-interactive
    ...
    if not openspec_args:
        ...
        console.print(
            "[dim]Usage: fsc-assistant openspec run <openspec-command> [args][/dim]"
        )
        console.print("[dim]Example: fsc-assistant openspec run --help[/dim]")
        sys.exit(1)
    ...
    if args:
        ...
        exit_code = run_openspec_command(
            subcommand, remaining_args, interactive=interactive
        )
    else:
...
def version_command() -> None:
    ...
    Examples:
        ...
        fsc-assistant openspec version
    """
    is_installed, version = check_openspec_installed()

    if not is_installed:
        console.print("[red]✗ OpenSpec is not installed[/red]")
        console.print("[dim]Run 'fsc-assistant openspec init' to install[/dim]")
        sys.exit(1)
    ...
    if version:
        ...
        console.print(
            "[yellow]OpenSpec is installed but version could not be determined[/yellow]"
        )
...
def install_command() -> None:
    ...
    Examples:
        ...
        # Install OpenSpec
        fsc-assistant openspec install
